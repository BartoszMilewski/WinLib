...
FileViewReadOnly::FileViewReadOnly (std::string const & path)
	  ...
	  _viewSize (0)
{
	...
	_viewSize = _end.ToMath () < 0x100 * granularity ? _end.Low () : 0x100 * granularity;
	_viewBuf = reinterpret_cast<char *> (::MapViewOfFile (_hMap,
														  FILE_MAP_READ,
														  _cur.High (),
														  _cur.Low (),
														  _viewSize));
