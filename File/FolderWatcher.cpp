	  ...
	  _refresh (true)
{
    ...
    {
		// non-recursive
		_folders.push_back (std::auto_ptr<WatchedFolder> (new WatchedFolder (*it, false)));
    }
}
...
void MultiFolderWatcher::AddFolder (std::string const & folder, bool recursive)
{
	...
	if (it == _folders.end ())
	{
		...
		_folders.push_back (std::auto_ptr<WatchedFolder> (new WatchedFolder (folder, recursive)));
		_refresh = true;
...
void MultiFolderWatcher::AddFolders (std::vector<std::string> const & folders)
{
		 ...
		 ++newFolder)
	{
		...
		if (it == _folders.end ())
		{
			...
			_folders.push_back (std::auto_ptr<WatchedFolder> (new WatchedFolder (*newFolder, false)));
			_refresh = true;
		}
	}
	if (_refresh)
