...
template <class T>
void auto_vector<T>::append (auto_vector<T> & arr)
{
	size_t appendSize = arr.size ();
	if (appendSize == 0)
		return;

	size_t newSize = _arr.size () + appendSize;
	_arr.reserve (newSize);
	for (size_t i = 0; i < appendSize; i++)
	{
		_arr.push_back (arr._arr [i]);
	}
	arr._arr.clear ();
}
